...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-11 px-4 text-left align-middle text-xs font-semibold tracking-[0.1em] uppercase text-zinc-400",
        className
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn("px-4 py-3 align-middle text-zinc-200", className)}
      {...props}
    />
  )
}
